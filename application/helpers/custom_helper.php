...
if (!function_exists('dashboard_url')) {
    ...
    {
        return base_url() . get_route('dashboard', true);
    }
}
    ...
    function generate_category_url($category)
    {
        ...
        if (!empty($category)) {
            if ($category->parent_id == 0) {
                ...
                return base_url() . $category->parent_slug . "/" . $category->slug;
            }
        }
    }
}
...
if (!function_exists('generate_post_url')) {
    ...
    {
        if (!empty($post)) {
            return base_url() . get_route("blog", true) . $post->category_slug . '/' . $post->slug;
        }
    }
}
...
if (!function_exists('generate_profile_url')) {
    ...
    {
        if (!empty($slug)) {
            return base_url() . get_route("profile", true) . $slug;
        }
    }
}
...
if (!function_exists('generate_url')) {
    ...
    {
        if (!empty($route_2)) {
            return base_url() . get_route($route_1, true) . get_route($route_2);
        } else {
            return base_url() . get_route($route_1);
        }
    }
}
...
if (!function_exists('counts')) {
    function counts()
    {
        if(is_admin()){
            $ci =& get_instance();
            return $ci->settings_model->counts();
        }else{
            $data=array(
                'payouts'=>0,
                'orders'=>0,
