    ...
    function get_product_form_data($product)
    {
        ...
        if (!empty($product)) {
            $disabled = ""; 
                ...
                $data->button = '<button class="btn btn-md btn-block btn-product-cart"' . $disabled . '><span class="btn-cart-icon"><i class="icon-cart-solid"></i></span>' . "Sepete Ekle" . '</button>'; 
        }
        return $data;
    }
}
...
if (!function_exists('get_order_products')) {
    function get_order_products($ci, $order_number)
    {
        return $ci->order_model->get_order_products($order_number);
    }
}
...
if (!function_exists('price_formatted')) {
    function price_formatted($price,  $convert_currency = false)
    {
        $ci =& get_instance();
        $price = $price / 100;
...
if (!function_exists('price_decimal')) {
    function price_decimal($price, $currency_code, $convert_currency = false)
    {
        $ci =& get_instance();
        //convert currency
