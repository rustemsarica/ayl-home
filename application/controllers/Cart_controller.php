...
class Cart_controller extends CI_Controller
{
    ...
    {
        $data['title'] = "sepet";
        $data['description'] = "shopping_cart";
        $data['keywords'] = "shopping_cart";

        $data['cart_items'] = $this->session_cart_items;
        $data['cart_total'] = $this->cart_model->get_sess_cart_total();
        $data['cities']= $this->settings_model->get_cities();
    ...
    public function add_to_cart()
    {
        ...
        if (!empty($product)) {
            if ($product->status != 1) {
                $this->session->set_flashdata('product_details_error', "Ürün şuan da satışta değildir.");
            } else {
    ...
    public function shipping()
    {
        ...
        $data['states']=$this->location_model->get_cities_tr();
        ...
        */
        ...
        $result = $this->shipping_model->calculate_cart_shipping_total_cost($cart_items);
    ...
    {   /*
        ...
        $vars=null;
        $data['result'] = 1;
        $data['html_content'] = $this->load->view('cart/_shipping_methods', $vars, true);
        echo json_encode($data);
    }
