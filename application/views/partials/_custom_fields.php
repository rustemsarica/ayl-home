...
<?php if (!empty($custom_fields)):
    foreach ($custom_fields as $custom_field):
        if (!empty($custom_field)):
            $custom_field_name = parse_serialized_name_array($custom_field->name_array, 4);
            ...
            <?php elseif ($custom_field->field_type == "number"):
                $input_value = $this->field_model->get_product_custom_field_input_value($custom_field->id, $product->id); ?>
                <div class="col-sm-12  col-sm-12 col-custom-field">
                    <label><?= $custom_field_name; ?></label>
            ...
            <?php elseif ($custom_field->field_type == "dropdown"): ?>
                <div class="col-sm-12 col-sm-12 col-custom-field">
                    ...
                    <select name="field_<?= $custom_field->id; ?>" class="form-control custom-select" <?= ($custom_field->is_required == 1) ? 'required' : ''; ?>>
                        ...
                        <?php $field_options = $this->field_model->get_field_options($custom_field, 4);
                        $field_values = '';
            ...
            <?php elseif ($custom_field->field_type == "checkbox"): ?>
                <div id="checkbox_options_container_<?= $custom_field->id; ?>" class="col-sm-12  "col-sm-12"; ?> col-custom-field checkbox-options-container" data-custom-field-id="<?= $custom_field->id; ?>">
                    <label><?= $custom_field_name; ?></label>
                    <div class="row">
                        <?php $field_options = $this->field_model->get_field_options($custom_field, 4);
                        $field_values = '';
